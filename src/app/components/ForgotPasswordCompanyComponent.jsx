...
import { forgotPasswordforcompanyusers, isAuth } from '../../../actions/auth';
...
const ForgotPasswordCompanyComponent = () => {
  ...
  const { error, message, loading, Company_email } = values;
  ...
  const handleSubmit = (e) => {
    ...
    console.log({ ...values })
    forgotPasswordforcompanyusers({ Company_email }).then(data => {
      if (data.error) {
        setValues({ ...values, error: data.error });
      } else {
        setValues({ ...values, message: data.message, Company_email: '' });
      }
    });
  }
              ...
              <form className='md:flex mdd:flex md:flex-col mdd:flex-col flex flex-col gap-4' onSubmit={handleSubmit}>
                <div className='py-4 grid grid-cols-1 gap-4'>
                  <div className='relative mt-2'>
                    <input value={Company_email} type="text" onChange={handleChange("Company_email")} className="block p-2 rounded-xl px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 bg-gray-50 dark:bg-gray-700 border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                    <label className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:text-purple-950 peer-focus:dark:text-purple-950 peer-focus:font-bold peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 before:content-[' '] after:content-['*'] after:text-red-500 after:ml-1 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">Registered Company Email</label>
                  </div>
                </div>
...
export default ForgotPasswordCompanyComponent